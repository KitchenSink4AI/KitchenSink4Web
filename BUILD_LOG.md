...
## 2026-09-10 (v1.0.1, the registry debut, and a day of lane C in the field)

v1.0.1 released from main via PR #1: get_server_info new in the LITE CORE
(present under the shipped read-only default; counts from the live registry,
test figures shipped as measured package data in figures.json), the
self-identification instructions line, the survivors() liveness fix (wait
object answers liveness; creation time keeps defeating PID reuse), the
canonical dual-license block in NOTICE.md and README, LICENSE copyright line,
PyPI author to Alvut Consulting, LLC, fonts self-hosted. Suites: unit 1,329/4,
browser 731/1/1xpass, gates 92, zero failed, nothing skipped for missing
captures. The release also DEBUTED the MCP registry record - the search had
returned zero servers for this product until today.

The PR surfaced the CLA infrastructure gap: the cla-signatures branch never
existed (created), and the Co-Authored-By Claude trailer counts as an
unsigned committer no allowlist could clear - merged by author admin
authority, then fixed properly family-wide by allowlisting the trailer while
the CLA stays required. The ext/integrate branch back-merged main the same
day: figure conflicts resolved by fresh measurement (2,369 collected: 1,547
unit, 822 browser), figures.json regenerated by its own writer, pyproject
shipping both the extension assets and figures.json, guards 38/38.

Field day for lane C against the author's real Firefox: the full-mode
recorder crash fixed (recorders skip stream-less contexts; list_requests/
list_console/get_page_errors refuse with the lane named), the remember-for-30-
minutes grants no longer wiped by any session close (structural pin added),
the OWA composer invisibility root-caused live to its endgame: OWA renders
its entire right-hand surface (reading-pane content AND composer) outside
the landmarks the segmenter owns - panel machinery fires, GROUP_HOPS was not
the cause (12 tested at the desk, reverted), exclusion survives 16k budgets.
Injection quarantine and zero-width stripping both observed working on real
mail. Remaining lane C gaps ledgered: unscoped get_text and find_elements hit
the main_frame path, tab open/adopt unbuilt, auth heuristic false-negatives
on OWA. Logged at 2026-09-10 18:12.

## 2026-09-13 to 2026-09-17 (community edition wave: the labelling, and v1.0.2)

Six days of main history, logged late and in one pass. The first two days were
the product page: the Web service desk was routed the way the other three pages
route theirs, the page gained the view counter its siblings have, the desktop
and mobile layouts were polished, and the install section was rebuilt over seven
commits across 09-13 and 09-14. Codex stdio setup joined the client list, the
installation cards were unified and then made self-contained and collapsed, the
desktop steps became explicit with direct downloads, platform support was
clarified, and an independent review produced one more copy pass on top of the
approved UX pass.

09-15 was hardening. Publishing to PyPI now runs only from a GitHub Release, so
there is one route and it is auditable. The star request asks once, on first
start, and nowhere else. The community README and the docs footer point at the
Business edition. The docs parity pass gave llms.txt five refusal codes it was
missing and the tail section it never had, and added the Python version
classifiers the PyPI metadata never carried. The licence audit fixes followed:
LICENSE is now byte-exact to the canonical AGPL-3.0 text from gnu.org (sha256
0d96a4ff68ad6d4b6f1f30f713b18d5184912ba8dd389f86aa7710db079abcb0, 34,523 bytes,
661 lines), which is what moves GitHub's detector off NOASSERTION, since
licensee resolves on exact content rather than similarity. The prepended
copyright line that caused it moved to NOTICE.md, and the pyproject comment
stopped overclaiming what the licence grants.

The registry and annotation slice landed early on 09-16: a title annotation on
every registered tool, destructiveHint, idempotentHint and openWorldHint on
every tool, privacy policy, contact and documentation URLs in the bundle
manifest, the org registry namespace reserved in the README mcp-name line, the
README privacy section moved to where it claimed to be, and the README itself
rewritten outcome first, install first, with the Business edition on the first
screen. Two fixes came out of that slice rather than out of the plan. The
annotation guard was leaking the process read-only grade into the tests that ran
after it, so a suite's result depended on its order. The tracked `.mcpb` was
repacked after the manifest edit, since the bundle carries its own copy of the
manifest and the two had drifted apart the moment the URLs went in. Token
figures were then re-measured against what the client actually receives.

The parity audit against the Business branch is the one that came back clean.
Everything on the web Business branch is genuinely Business-only, so unlike
Word, Excel and PowerPoint this edition was not missing a product fix, and 1.0.2
carries the annotations, the labelling and the licence work rather than a
catch-up.

Then the edition got its name. The page title, the hero install button, the
.mcpb download button and the README title line all say Community Edition, a
stylised badge sits under the wordmark, and the masthead gained a BUSINESS
EDITION link. The seven translated pageTitle strings were relabelled too, since
apply() overwrites document.title on every language switch and changing only the
tag would have lost the label the moment a reader picked a language. User-facing
prose moved to US spelling, licence to license, with identifiers and wire values
untouched. The storefront's Community editions banner went in above the hero,
its Business link became a real button rather than bordered text, and the
language toggle moved off the page-local `ks4web-lang` key onto the shared
`ks4-lang` key with a `?lang=` parameter honoured, so a visitor's choice follows
him across the four product pages. CHANGELOG gained its 1.0.2 entry, written as
`### 1.0.2` to match the file's own shape rather than the dated heading the
brief asked for, since no entry in it has ever carried a date.

The release was blocked for several hours by the CLA gate, family-wide. The four
release branches each carried seven commits authored
`Claude <noreply@anthropic.com>`, which GitHub resolves to the lowercase login
`claude`, and contributor-assistant matches its allowlist against the login
case-sensitively, so the `Claude*` entry never fired. The owner ruled re-author
rather than widen the allowlist: every commit in the range was rebuilt with
`git commit-tree`, preserving trees, parents, messages, author dates and
committer dates, seven of twenty-two re-authored to the owner, with `git diff`
old head against new head empty before the force-push. The check then passed
with no workflow change, and the allowlist afterwards dropped `Claude*` and
`Codex*` entirely, leaving the owner and bots.

v1.0.2 published 2026-09-16 21:19 KST with `kitchensink4web-1.0.2.mcpb`
attached, publish run 35095185064 green, PyPI confirmed at 1.0.2. Suite on the
final tree: 1,712 passed, 0 failed, 311 skipped in 10m00s, identical to the
baseline. PRs #2 through #5 merged in the same hour, #2 carrying the release and
the rest the link fixes that followed it: the showroom's eight .mcpb links
repointed at the new tag, and the switchbar storefront tile and the masthead
FAMILY link moved off /products/community/, the corporate page in the dark
register, onto /products/, the showroom that actually presents the family. Lane
C and the extension work continue on ext/integrate and are not in any of this.
Logged at 2026-09-17 00:51.
