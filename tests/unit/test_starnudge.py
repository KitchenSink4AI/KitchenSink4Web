...
from kitchensink4web.starnudge import (
    MARKER_NAME,
    MESSAGE,
    announce_once,
    marker_path,
...
def state(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("KS4WEB_STAR_NUDGE", raising=False)
    return tmp_path
...
def test_the_toggle_silences_it(state, monkeypatch):
    monkeypatch.setenv("KS4WEB_STAR_NUDGE", "off")
    out = io.StringIO()
...
def test_an_unwritable_state_directory_is_silence_not_a_crash(state, monkeypatch):
    monkeypatch.setattr(
        "kitchensink4web.starnudge.marker_path",
