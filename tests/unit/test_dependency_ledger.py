...
DESIGN 10.3 rule 1 and PLAN Phase 0: no copyleft or source-available
dependency anywhere in the required install, with the rule enforced by a
test. A dependency added to pyproject.toml and not to DEPENDENCY_LEDGER.md
fails here, which is the only way a ledger stays true past the first week.
...
#: but you can read it" is the trap Notte's SSPL relicense sprang on the
#: category (DESIGN 10.1).
FORBIDDEN_IN_REQUIRED = (
    "gpl", "agpl", "lgpl", "sspl", "bsl", "busl", "elastic license",
...
def test_ledger_exists():
    assert LEDGER.is_file(), (
        "DEPENDENCY_LEDGER.md is required from Phase 0 (DESIGN 10.3 rule 1)"
    )
...
def test_the_ledger_records_the_bidi_provenance_decision():
    """DESIGN 10.3 rule 2, made now because it is cheap now and expensive
    later. A future session must find this decision here rather than
    re-deciding it against Playwright's convenient sources."""
    text = LEDGER.read_text(encoding="utf-8")
    assert "W3C" in text and "not lifted" in text
