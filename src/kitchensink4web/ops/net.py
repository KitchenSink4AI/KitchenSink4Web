...
async def set_routing(
    ...
    if action == "modify":
        return await _modify(sess, state, origin, header, value,
                             secret_ref, jar=jar)
    if action == "strip_params":
        return await _strip_params(sess, state, params, preset, jar=jar)
...
async def _modify(sess, state, origin, header, value, secret_ref, *,
                  jar=None) -> dict:
...
async def _modify_credential(sess, state, named, name, value,
    ...
    await _install_modify(sess, state, named, name,
                          _credentials.secret_value(ref),
                          source=f"secret_ref:{ref}", jar=jar)
    return {"session": sess.session_id, "action": "modify",
            "routing": _state_view(state)}
...
async def _install_modify(sess, state, named, name, header_value, *,
    ...
    # THE SEAM (integration 2026-09-08): set_routing resolves ONE jar and
    # refuses rather than guessing when a session has several, so the two
    # consent-wave installers take that jar too. Routing the focused jar
    # while every sibling action refuses would be the same silent
    # wrong-jar answer the multi-context wave exists to close.
    await (jar or sess).context.route(_matcher, _add_header)
...
async def _strip_params(sess, state, params, preset, *, jar=None) -> dict:
    ...
    await (jar or sess).context.route("**/*", _strip)
