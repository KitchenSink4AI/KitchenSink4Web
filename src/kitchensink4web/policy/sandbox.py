"""Opt-in path sandboxing governed by the KS4WEB_ALLOWED_ROOTS environment
variable.

Copied from word-mcp `core/sandbox.py`, which is the family's proven
canonicalization-then-containment implementation, with the env var renamed
and the domain notes rewritten. The algorithm is unchanged on purpose: it
has already survived the Word and Excel adversarial rounds, and re-deriving
a path-escape check is how you introduce a path-escape bug.

**What it governs in the browser domain** (PLAN 1.1): the download
directory, the spill-to-file directory for oversized payloads, the audit
directory, and the seeded-profile directory. Those are the only four places
KS4Web touches a filesystem, and all four are places an injected page
instruction would like to reach.

The honest framing, copied from playwright-mcp's own phrasing on its file
guardrail because it is the right one: this is a convenience defense to
catch unintended file access, not a secure boundary.

When KS4WEB_ALLOWED_ROOTS is unset (or empty), nothing here restricts anything:
check_path returns its input untouched. When it is set to an
os.pathsep-separated list of directories (Windows example:
"C:\\Users\\me\\Downloads;D:\\Work"), every filesystem path the server
...
ENV_VAR = "KS4WEB_ALLOWED_ROOTS"


class SandboxViolation(WebMcpError):
    """A path resolved outside the allowed roots; nothing was read or
    written. KS4WEB_ALLOWED_ROOTS governs which directories are allowed."""
